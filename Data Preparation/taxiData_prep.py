...
Given large volume of data, chunking option is utilized to reduce memory load. 

The polyline trajectory is also coverted to location vector that is easier to 
process with ElasticSearch.

Todo:
    * Compress output JSON data
    * Handle mid-stream failure of processing to avoid procession from beginning
    of the file
"""
...
chunkReader = pd.read_table(".//SampleData//Porto_taxi_data_test_partial_trajectories.csv", \
sep=',', header=0, names=colNames, dtype=colDtype, chunksize=50000)
...
for chunk in chunkReader:
    ...
    # and then stack those horizontally to create new column with counter for
    ...
    rowChunk.to_json(".//SampleData//FormattedJSON//rowChunk_"+str(counter)+".json",\
    orient='records')
